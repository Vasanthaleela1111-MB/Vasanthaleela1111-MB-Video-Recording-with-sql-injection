...
<main>
    ...
    <!-- <button id="listVideosBtn">🎥 List Videos</button>
<select id="videoDropdown" style="display:none; margin-left:10px;"></select> -->
    <br><br>
...
//     fetch('https://18.189.144.42/dev1/sms/index.php/otp/list_user_videos')
//         .then(res => res.json())
//         .then(data => {
...
//                     videoCard.innerHTML = `
//                         <video width="180" height="100" style="border-radius:5px;" src="${video.video_url}" muted></video>
//                         <p style="font-size:12px; color:#555;">${video.created_at}</p>
//                     `;
//                     videoCard.onclick = () => {
//                         verifyAndPlayVideo(video.video_url);
//                     };
//                     container.appendChild(videoCard);
//                 });
//             } else {
//                 container.innerHTML = '<p>No saved videos found.</p>';
//             }
//         })
//         .catch(err => {
//             console.error('Error fetching history:', err);
//             document.getElementById('videoHistory').innerHTML = '<p>Error loading videos.</p>';
//         });
// }

// function verifyAndPlayVideo(videoUrl) {
//     // Reuse your existing OTP verification flow
//     document.getElementById('authSection').innerHTML = `
//         <button id="verifyBtn">Verify & Play</button>
//     `;
//     document.getElementById('verifyBtn').onclick = () => {
//         // Here, replace `videoBlobUrl` with `videoUrl`
//         startOtpVerification(videoUrl);
//     };
// }

// // Modify OTP verification so it accepts a URL
// function startOtpVerification(videoUrl) {
//     fetch('https://18.189.144.42/dev1/sms/index.php/otp/generate_qr', {
//         method: 'POST',
//         headers: {'Content-Type': 'application/x-www-form-urlencoded'},
//         body: new URLSearchParams()
...
//                 <img src="${qrCode}" width="200"><br><br>
//                 <input type="text" id="totp_code" placeholder="Enter 6-digit code"><br><br>
//                 <button id="submitOtp">Submit & Play</button>
//             `;
//             document.getElementById('submitOtp').onclick = () => {
//                 const code = document.getElementById('totp_code').value;
//                 if (!code) return alert("Please enter the code!");
...
//                         });
//                     } else {
//                         Swal.fire({ icon: 'error', title: 'Verification Failed', text: 'Invalid or suspicious input.' });
//                     }
//                 });
//             };
//         }
...
document.getElementById('listVideosBtn').onclick = function () {
    fetch('https://18.189.144.42/dev1/sms/index.php/otp/list_user_videos')
        .then(res => res.json())
        .then(data => {
            ...
            if (data.status === 'success' && data.videos.length > 0) {
                ...
                dropdown.innerHTML = `<option value="">-- Select a Video --</option>`;
                data.videos.forEach(video => {
                    dropdown.innerHTML += `<option value="${video.video_url}">${video.created_at}</option>`;
                });
            } else {
                alert("No saved videos found.");
            }
        })
        .catch(err => console.error('Error loading videos:', err));
};

// When a video is selected from dropdown
document.getElementById('videoDropdown').onchange = function () {
    const selectedUrl = this.value;
    if (!selectedUrl) return;
    startOtpVerification(selectedUrl);
};
...
function startOtpVerification(videoUrl) {
    ...
    .then(qrData => {
        if (qrData.status === 'success') {
            const secret = qrData.secret;
            document.getElementById('authSection').innerHTML = `
                <p>Scan QR & Enter OTP:</p>
                <img src="${qrData.qr_code}" width="200"><br><br>
                <input type="text" id="totp_code" placeholder="6-digit code"><br><br>
                <button id="submitOtp">Submit & Play</button>
            `;
            document.getElementById('submitOtp').onclick = () => {
                const code = document.getElementById('totp_code').value;
